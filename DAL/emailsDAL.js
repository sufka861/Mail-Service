...
function addToSentJason(mailOptions) {
  ...
  fs.writeFile(
    path.join(process.cwd(), paths.emailsSentPath),
    JSON.stringify(emailsSent),
    "utf-8",
    (err) => {
      if (err) throw err;
      else console.log("email saved to emailsSent.json");
    }
  );
}
...
function addToFutureEmails(mailOptions, timeToSend) {
  ...
  fs.writeFile(
    path.join(process.cwd(), paths.emailsToSendPath),
    JSON.stringify(emailsToSend),
    "utf-8",
    (err) => {
      if (err) throw err;
      else console.log("email saved to emailsToSend.json");
      return mailID;
    }
  );
}
...
function deleteFromFutureEmails(mailID) {
  ...
  fs.writeFile(
    ...
    (err) => {
      if (err) throw err;
      else console.log("deleted email from emailsToSend.json");
    }
  );
}
